;(function(){
  ...
  new Vue({
    ...
    methods:{
      ...
      remove: function(id){
        var index = this.find_index(id);       /*删*/
        this.list.splice(index,1);
      },
      next_id: function(){
        return this.list.length + 1;          /*每push一个就多加一个id*/
      },
      ...
      find_index:function(id){
        return this.list.findIndex(function(item){    /*遍历ID*/
          return item.id == id;
        });
      },
    ...
    watch: {
      list: {
        deep:true,
        handler: function(new_val,old_val){      /*新旧值替换*/
          if(new_val){
            ms.set('list', new_val);
          }else{
            ms.set('list', []);
